...
already_allocated( buffer *buf, size_t length )
{
	...
	front_len = ( size_t ) front_length_of( buf );
  	...
  	return ( buf->length >= required_len );
}
...
duplicate_buffer(buffer *buf)
{
  ...
  if ( old_buffer->length == 0 ) {
    //pthread_mutex_unlock( old_buffer->mutex );
    return NULL;
  }
  new_buffer = alloc_buffer_with_length(old_buffer->length);
  memcpy( new_buffer->top, old_buffer->top, old_buffer->length );
