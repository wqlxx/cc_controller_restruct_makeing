...
compare_string( const void *x, const void *y ) {
  return strcmp( x, y ) == 0 ? CC_E_NONE : CC_E_ERR;
}
...
compare_mac( const void *x, const void *y ) {
  return memcmp( x, y, OFP_ETH_ALEN ) == 0 ? CC_E_NONE : CC_E_ERR;
}
...
compare_uint32( const void *x, const void *y ) {
  return *( ( const uint32_t * ) x ) == *( ( const uint32_t * ) y ) ? CC_E_NONE : CC_E_ERR;
}
...
compare_datapath_id( const void *x, const void *y ) {
  return *( ( const uint64_t * ) x ) == *( ( const uint64_t * ) y ) ? CC_E_NONE : CC_E_ERR;
}
